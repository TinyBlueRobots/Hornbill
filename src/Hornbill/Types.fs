...
type Response =
  ...
  static member WithStaticFile path = Response.WithBytesAndHeaders(200, File.ReadAllBytes path, [| System.IO.Path.GetFileName path |> sprintf "Content-Disposition:attachment; filename=%s" |] )
