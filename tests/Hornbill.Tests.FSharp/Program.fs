...
let headers() =
  use testService = new TestService()
  Response.withHeaders 200 [ "foo", "bar" ] |> testService.Service.AddResponse "/foo" Method.GET
  let response = testService.Client.GetAsync("/foo").Result
  response.Headers.GetValues("foo") |> Seq.head == "bar"

let dlg() =
  use testService = new TestService()
  Response.withDelegate (fun _ -> Response.withStatusCode 200) |> testService.Service.AddResponse "/foo" Method.GET
...
"""
  ...
  let response = (new HttpRequestMessage(HttpMethod methd, "statuscode") |> testService.Client.SendAsync).Result
